...
double fSumErfExpTotal(double* xVar, double* par)
{
	...
	fpart1 = norm1*(ratio1*erfpart1 + exppart1);
	...
	fpart2 = norm2*(ratio2*erfpart2 + exppart2);
	...
	fpart3 = norm3*(ratio3*erfpart3 + exppart3);
	...
	fpart4 = norm4*(ratio4*erfpart4 + exppart4);
	...
	fpart5 = norm5*(ratio5*erfpart5 + exppart5);
...
double fSumErfExpTotalFixed(double* xVar, double* par)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		result += norm[i]*(ratio[i]*erfpart[i] + exppart[i])*decay[i];
	}
